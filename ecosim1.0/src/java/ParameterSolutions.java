...
/**
  ...
  *
  */
public class ParameterSolutions extends JFrame {

    /**
     *  ParameterSolutions constructor.
     */
     ...
     */
    public ParameterSolutions(String[] args) {
        boolean nogui = false;
        masterVariables = new MasterVariables();
        helpAbout = new HelpAboutWindow(masterVariables);
        log = masterVariables.getLog();
        narr = masterVariables.getNarrator();
        workingDirectory = masterVariables.getWorkingDirectory();
        nogui = checkArguments(args);
        execs = new Execs(masterVariables);
        masterVariables.setExecs(execs);
        //initComponents();
        if (nogui) {
            runEverythingWithoutGui(args);
        }
        else {
            initComponents();
        }
    ...
    /**
     *  Start an instance of ParameterSolutions.
     *
     *  @param args The command line arguments.
     */
    ...
    private boolean checkArguments (String[] args) {
        ...
        errorMessage += "    -n, --nogui   : Runs everything without the gui.\n";
        for (String arg: args) {
            ...
            else if (arg.equals("-n") || arg.equals("--nogui")) {
                error = true;
                //runEverythingWithoutGui(args);
            }
            // else {
                ...
                if (returnVal == FileChooser.APPROVE_OPTION) {
                    ...
                    if (! inputFasta.isValid()) {
                        ...
                        if (chooseTree() != -1) {
                             Thread thread = new Thread() {
                                public void run() {
                                    new DemarcationsAuto(hClimbResult, values, inputFasta, tree, masterVariables, "gui").setVisible(true);
                                }
                            };
                            thread.start();
                        }
    ...
    private int chooseTree() {
        ...
        int useNewick = JOptionPane.showOptionDialog(ParameterSolutions.this,
    ...
    /**
    * Here we can create a private method called from the check arguments parameter
    * that has the capability to run through everything. I think once it's finished
    * we can then pick things up from the working directory...
    * THE ORDER MATTERS OF THE ARUGMENTS
    * a typical run will look like:
    * java -jar bin/EcoSim.jar -n inputFile tree userSortPercentage id
    * 
    **/

    private void runEverythingWithoutGui(String[] args) {
        
        File startingFile = new File(args[1]);
        inputFasta = new Fasta(startingFile);
        noOutgroupFasta = removeOutgroup(inputFasta);
        // Usually 3 which is 1.25x
        final int userChoice = Integer.parseInt(args[3]);
        tree = new File(args[2]);
        final String id = args[4];
        ...
        Thread thread = new Thread() {
            public void run() {
                if (running) {
                    ...
                    values.run();
                    hClimbResult =  hillClimbing(values.getValue());
                    // I don't think we have to run these?
                    //runNpopConfidenceInterval();
                    //runSigmaConfidenceInterval();
                    //runOmegaConfidenceInterval();
                    //runDriftConfidenceInterval();
                    readyForCI = true;
                    printResults();
                    new DemarcationsAuto(hClimbResult, values, inputFasta, tree, masterVariables, id).setVisible(true);
                    running = false;
                }
            }
        };
        // String test = "log append test: HELLO";
        // test += args[0];
        // test += ",";
        // test += args[1];
        // test += args[2];
        // log.append(test+"\n");
        // narr.println("narr println test: HELLO!");
        thread.start();
        return;
    }
     ...
     *
     *  @param inFile The FASTA formated input file.
     ...
     *
     *  @paramsavedFile A previously saved file from this program.
