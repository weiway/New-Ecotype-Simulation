...
program binningdanny
  ...
  if (iargc() .ge. 3) then
    ! Expect filename for identitymatrix.dat, binlevels.dat, and output.dat.
  ...
  inquire (file = trim (identitymatrix_dat), exist = file_exists)
  if (file_exists .neqv. .true.) then
    write (unit = *, fmt = *) "The identitymatrix.dat file was not found at: ", trim (identitymatrix_dat)
    stop
  end if
  inquire (file = trim (binlevels_dat), exist = file_exists)
  if (file_exists .neqv. .true.) then
    write (unit = *, fmt = *) "The binlevels.dat file was not found at: ", trim (binlevels_dat)
    stop
  end if
  ! Open Input files
