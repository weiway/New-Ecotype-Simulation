...
program removegaps
  ...
  if (iargc() .ge. 3) then
    ! Expect filenames for sequencesfasta.txt, numbers.dat, and fasta.dat.
    ...
    sequencesfasta_txt = 'sequencesfasta.txt'
  ...
  if (file_exists .neqv. .true.) then
    write (unit = *, fmt = *) "The sequencesfasta.txt file was not found at: ", trim (sequencesfasta_txt)
    stop
  end if
  inquire (file = trim (numbers_dat), exist = file_exists)
  if (file_exists .neqv. .true.) then
    write (unit = *, fmt = *) "The numbers.dat file was not found at: ", trim (numbers_dat)
    stop
  end if
  ! Open Input files
