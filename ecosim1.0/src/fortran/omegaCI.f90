...
program omegaCI
  ...
  ! Expect path and filename for: omegaIn.dat omegaOut.dat
  if (iargc() .ge. 2) then
    call getArgument (1, inputFile)
    call getArgument (2, outputFile)
  else
    write (unit = *, fmt = *) "Invalid number of parameters supplied!"
  ...
  if (numincs%omega .eq. 0) then
    top%omega = bottom%omega * 10.0
    diffomega = log (top%omega) - log (bottom%omega)
    numincs%omega = 1
    xincs = 0
  endif
  do indexomega = 0, numincs%omega
    parameters%omega = exp (log (bottom%omega) + indexomega * (diffomega / numincs%omega) * 0.999)
    params(1) = log (parameters%sigma)
    params(2) = parameters%npop
    step(1) = log (parameters%sigma) / 2.0
    step(2) = parameters%npop / 2.0
    if (log (parameters%sigma) .lt. 0.3 .and. log (parameters%sigma) .gt. -0.3) step(1) = 0.15
    yvalue = 0.0
