...
public class Demarcation implements Runnable {
    ...
    private void findEcotypes(NewickTreeNode node,
		...
		//System.out.println(ES1switch);
        if (node.isLeafNode()) {
			System.out.println("Reach a leaf node");
            String name = node.getName();
            if (! name.equals(phylogeny.getOutgroupIdentifier())) {
				System.out.println("ECOTYPE FOUND! - Switch back to ES2");
                sequences.add(name);
            ...
            if (sequences.size() == 0) {
				System.out.println("Sequences size is zero");
                return;
            }
            ...
            DemarcationConfidenceInterval demarcConf =
                ...
                sampleBinning, hillclimb.getResult(), suffix
            );
			...
			if (ES1switch == false) {
				demarcConf.run();
				if (demarcConf.getResult() <=10){
					ES1switch = true;
					System.out.println("Switch to ES1");
					//findEcotypes(node,parentHillclimbResult,iteration);
		            if (demarcConf.run2()[1] == 1) {
						System.out.println("Running ES1 - ECOTYPE FOUND! - Switch back to ES2");
		                ecotypes.add(sequences);
						ES1switch = false;
		            }
		            else {
		                ArrayList<NewickTreeNode> children = node.getChildren();
						System.out.println("Keep running ES1");
		                for (int i = 0; i < children.size(); i ++) {
		                    iteration ++;
		                    findEcotypes(
		                        children.get(i), hillclimb.getResult(), iteration
		                    );
		                }
					}
				}
	                ...
	                ArrayList<NewickTreeNode> children = node.getChildren();
					System.out.println("Running ES2");
	                for (int i = 0; i < children.size(); i ++) {
	                    iteration ++;
	                    findEcotypes(
	                        children.get(i), hillclimb.getResult(), iteration
