...
program hillclimb
  ...
  input_file = 'hillclimbIn.dat'
  output_file = 'hillclimbOut.dat'
  ...
  ! Read the input file.
  call readinput (trim (input_file), omega, sigma, npop)
  ...
  npop = nint (params(3))
  ! Output the answer.
  write (unit = output_unit, fmt = *) omega, sigma, npop, yvalue
...
subroutine callfredprogram (nparams, params, yvalue)
  ...
  if (params(2) .gt. 2.0) params(2) = 2.0
  if (params(1) .lt. -10.0) params(1) = -10.0
  if (params(2) .lt. -10.0) params(2) = -10.0
  omega = exp (params(1))
  sigma = exp (params(2))
  npop = nint (params(3))
  if (debug) then
