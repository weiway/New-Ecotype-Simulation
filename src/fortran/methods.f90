...
module methods
  ...
  subroutine binning (bin, div, crit, ncoalesce, numcrit, numanctot)
    ...
    nbins = 1
    lused = 0
    do 106 jcrit = 1, numcrit
      criterion = crit(jcrit)
      janc = numanctot - lused
      do
        div2 = 2 * (-0.75) * (exp((-4.0 / 3.0) * darrayGet (div, janc)) - 1.0)
...
106 continue
  ...
  subroutine doPeriodicSelection (activepop, numanctot, numstrain, &
    ...
    real                 :: x
    ! Allocate memory.
    allocate(eligiblepspop(activepop), stat = allocate_status)
    ...
    ! Deallocate memory.
    deallocate(eligiblepspop, stat = allocate_status)
    if (allocate_status .gt. 0) then
      write (unit = *, fmt = *) &
  ...
  subroutine randomClose ()
    ...
    deallocate(rng, stat = allocate_status)
    if (allocate_status .gt. 0) then
  ...
  subroutine randomInitialize (iii)
    ...
    allocate(rng(numberThreads), stat = allocate_status)
    if (allocate_status .gt. 0) then
  ...
  subroutine runProgram (omega, sigma, npop, numcrit, nu, nrep, &
    ...
    if (omega .ne. omega .or. omega .le. epsilon(0.d0) .or. &
      omega .ge. huge(0.0d0) - 1.0d0) return
    if (sigma .ne. sigma .or. sigma .le. epsilon(0.d0) .or. &
  ...
  subroutine simulation (omega, sigma, npop, numcrit, nu, lengthseq, &
    ...
    allocate(numstrain(nu), stat = allocate_status)
    if (allocate_status .gt. 0) then
    ...
    do
      ...
      call whichEventAndWhen (eligibleNI, eligiblePS, omega, sigma, time, &
        event)
    ...
    ! routine testforsuccessfit returns true for each success level when a
    ! replicate is within the tolerance range of all bins, etc.
  ...
  !>
  !> @param[in]     eligibleNI    The number of populations eligible for
  !>                                niche invasion.
  !> @param[in]     eligiblePS    The number of populations eligible for
  !>                                periodic selection.
  !> @param[in]     omega         The rate of niche invasion per eligible
  !>                                parental population, measured as niche
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine whichEventAndWhen (eligibleNI, eligiblePS, omega, sigma, time, &
    event)
    integer, intent(in)                :: eligibleNI
