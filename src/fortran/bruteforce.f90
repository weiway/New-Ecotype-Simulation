...
program bruteforce
  ...
  input_file = 'bruteforceIn.dat'
  output_file = 'bruteforceOut.dat'
  ...
  if (file_exists .neqv. .true.) then
    write (unit = *, fmt = *) &
      "The bruteforceIn.dat file was not found at: ", trim (input_file)
    ! Error, exit the program.
    stop
  end if
  ! Read the input file.
  call readinput (trim (input_file), omegabot, omegatop, sigmabot, &
  ...
  do while (indexomega .lt. numincsomega)
    ...
    do while (indexsigma .lt. numincssigma)
      ...
      do while (indexnpop .lt. numincsnpop)
        ...
        ! than zero.
        if (avgsuccess(1) .gt. 0) then
          write (unit = output_unit, fmt = 95) omega, sigma, npop, &
            (avgsuccess(i), i = 1, 6)
          if (debug) then
            write (unit = *, fmt = 95) omega, sigma, npop, &
              (avgsuccess(i), i = 1, 6)
          end if
        end if
...
95  format(f15.7,',',f15.7,',',i10,6(',',e18.10))
