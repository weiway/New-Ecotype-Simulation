...
program demarcationCI
  ...
  input_file = 'demarcationIn.dat'
  output_file = 'demarcationOut.dat'
  ...
  if (file_exists .neqv. .true.) then
    write (unit = *, fmt = *) &
      "The demarcationIn.dat file was not found at: ", trim (input_file)
    ! Error, exit the program.
    stop
  end if
  ! Read the input file.
  ...
  subroutine readinput (fname, omega, sigma, npop, istep, likelihood)
    ...
    ! sample following Acinas et al., this should be in the thousands.  The
    ! program is currently written to allow 10000 samples
