...
module Methods
    ...
    do
      ...
      select case (event)
        ...
        case (EVENT_PERIODIC_SELECTION)
		  Print *, "EVENT-PERIODICSECLECTION"
	  ...
	  Print *, "ntotalpop: ", ntotalpop
      if (ntotalpop .le. 1) exit
	  Print *, "ntotalpop > 1, keep running tier two loop--Tier Two------------------END"
	  Print *, "   "
    end do
	Print *, "ntotal <= 1 Finished running tier two loop------------------STOP TIER TWO!"
	Print *, "Divergenonultra"
  ...
  subroutine readAcinas (fname, acinas, bottom, top, numincs)
	...
	Print *,"bottom / top omega", bottom%omega, top%omega
	...
	Print *,"bottom / top sigma", bottom%sigma, top%sigma
  ...
  subroutine simulation (acinas, parameters, avgsuccess)
    ...
    avgsuccess = 0.0
	Print *, "Nrep: ", acinas%nrep
    do irep = 1, acinas%nrep
	  Print *, "STARTING FREDMETHOD=============Tier One=============START"
      call fredMethod (acinas, parameters, success)
	  Print *, "FINISHED FREDPROGRAM============Tier One===============END"
	  Print *, "                                       "
  ...
  subroutine whichEventAndWhen (parameters, activepop, event, numstrain, realname, time)
	...
	Print *,"RANDOM NUM2"
    call random_number (x)
	Print *,"effectiveOmege:" , effectiveOmega
	Print *,"effectiveSigma:" , effectiveSigma
